...
echo "Updating fendz_options table & running find/replace..."
mysql --user=$LOCAL_DB_USER --password="$LOCAL_DB_PASSWORD" <<EOF
	USE $LOCAL_DB;
	UPDATE fendz_options SET option_value="$LOCAL_URL" WHERE option_name="siteurl";
	UPDATE fendz_options SET option_value="$LOCAL_URL" WHERE option_name="home";
	UPDATE fendz_posts SET post_content = replace(post_content, '$REMOTE_URL', '$LOCAL_URL');
	UPDATE fendz_postmeta SET meta_value = replace(meta_value,'$REMOTE_URL','$LOCAL_URL');
EOF
errorcheck
